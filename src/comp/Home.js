...
export default ({categoryObj, tagCategory, tagObj, tagWorkMapper}) => {
  if (!categoryObj || !tagCategory || !tagObj || !tagWorkMapper) return null

  const ShortcutMenu = () => {
    const columns = MAIN_CATEGORY.map((categoryId, categoryIdIndex) => {
      const tagList = TagList({categoryId, tagTree: tagCategory[categoryId], depth: 2, tagObj, tagWorkMapper, categoryObj})
      return (
